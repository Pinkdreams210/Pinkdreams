...
class WEPOF_Product_Options_Frontend extends WEPOF_Product_Options_Utils {
	...
	******************************************************/
	...
	private function set_extra_options_in_wp_session_using_product_id($product_id, $extra_options){
		...
		if(is_array($extra_options) && !empty($extra_options)){
			$_SESSION[$SESSION_KEY] = $extra_options;
		}
	...
	private function get_extra_options_from_wp_session_by_product_id($product_id){
		...
		if(isset($_SESSION[$SESSION_KEY])){
			$extra_options = $_SESSION[$SESSION_KEY];	
	...
	***************************************************/
	public function woo_before_single_product(){
			...
			foreach($fields_hook_map as $hook_name => $fields){
				if($fields){
					foreach($fields as $field_name => $field){
						if( $this->is_valid_field($field) && $field->is_enabled() && $field->show_field($product->id, $categories) ){
		...
		//$this->set_fields_hook_map_in_session_using_product_id($product->id, $fields_hook_map);
		//$this->set_extra_options_in_session_using_product_id($product->id, $this->options_extra);
		$this->set_fields_hook_map_in_wp_session_using_product_id($product->id, $fields_hook_map);
	...
	private function get_product_options_extra_by_hook_name($hook_name){
		global $product;
		//$fields_hook_map = $this->get_fields_hook_map_from_session_by_product_id($product->id);
		$fields_hook_map = $this->get_fields_hook_map_from_wp_session_by_product_id($product->id);
		...
		if($extra_options){
			foreach($extra_options as $name => $data){
				if(isset($cart_item[$name])) {
					$custom_items[] = array( "name" => $data->get_title(), "value" => $cart_item[$name] );
				}
			}
		}

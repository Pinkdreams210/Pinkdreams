...
foreach ( $tabs as $name => $label ) {
    echo  '<a style="border-radius:5px;padding:12px 24px;background-color:#084B8A;color:#fff;" href="' . admin_url( 'admin.php?page=woo-product-customizerort&tab=checkout_page&report=' . $name ) . '" class="checkout-tab ' . (( $current_tab == $name ? 'nav-tab-active' : '' )) . '">' . $label . '</a>' ;
}
?>
			<-- Additional Options
		</h3>
